import { areSortedArraysEqual } from '@/utils/iteratees.utils';
...
export default function useHistoryBack(
                    ...
                    if (isHashChangedFromEvent.current) {
                        ...
                        if (hashes.length !== indexHashRef.current.length) {
                            if (delta > 0) {
                                ...
                                let realDelta = delta;
                                if (last) {
                                    const indexLast = historyState.currentIndexes.findIndex((l) => l === last.index);
                                    realDelta = historyState.currentIndexes.length - indexLast - 1;
                                }
                                historyState.isHistoryAltered = true;
                                window.history.go(-realDelta);
                                const removed = indexHashRef.current.splice(
                                    indexHashRef.current.length - delta - 1,
                                    delta,
                                );
    ...
    useEffect(() => {
        const handlePopState = (event: PopStateEvent) => {
            ...
            const index = i || 0;
            try {
                const currIndex = hashes
                    ? indexHashRef.current[indexHashRef.current.length - 1].index
                    : indexRef.current;
                ...
                if (
                    (!isClosed.current && (index === 0 || index === prev)) ||
                    (hashes && (index === 0 || index === prev))
                ) {
                    if (hashes) {
